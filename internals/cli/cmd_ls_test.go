...
func (s *PebbleSuite) TestLsExtraArgs(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"ls", "extra", "args"})
...
func (s *PebbleSuite) TestLsDirectory(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"ls", "-d", "/"})
	c.Assert(err, IsNil)
...
func (s *PebbleSuite) TestLsLongFormat(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"ls", "-l", "/"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
...
func (s *PebbleSuite) TestLsPattern(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"ls", "/foo/bar.*"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
...
func (s *PebbleSuite) TestLsInvalidPattern(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"ls", "/foo/ba[rz]/fail"})
