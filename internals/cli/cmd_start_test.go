...
func (s *PebbleSuite) TestStart(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"start", "srv1", "srv2"})
...
func (s *PebbleSuite) TestStartFails(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"start", "srv1", "srv3"})
...
func (s *PebbleSuite) TestStartNoWait(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"start", "srv1", "srv2", "--no-wait"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
