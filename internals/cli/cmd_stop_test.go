...
func (s *PebbleSuite) TestStop(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"stop", "srv1", "srv2"})
...
func (s *PebbleSuite) TestStopFails(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"stop", "srv1", "srv3"})
...
func (s *PebbleSuite) TestStopNoWait(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"stop", "srv1", "srv2", "--no-wait"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
