...
func (s *PebbleSuite) TestSignalShortName(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"signal", "HUP", "s1"})
...
func (s *PebbleSuite) TestSignalFullName(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"signal", "SIGHUP", "s2"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
}
...
func (s *PebbleSuite) TestSignalMultipleServices(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"signal", "SIGHUP", "s1", "s2"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
}

func (s *PebbleSuite) TestSignalErrorLowercase(c *check.C) {
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"signal", "hup", "s1"})
