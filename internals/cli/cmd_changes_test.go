...
func (s *PebbleSuite) TestChangesExtraArgs(c *check.C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"changes", "extra", "args"})
...
func (s *PebbleSuite) TestChangesAllDigitsSuggestion(c *check.C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"changes", "42"})
...
func (s *PebbleSuite) TestNoChanges(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"changes", "svc1"})
...
`
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"changes"})
...
`
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--abs-time", "42"})
	c.Assert(err, check.IsNil)
...
func (s *PebbleSuite) TestChangeSimpleRebooting(c *check.C) {
	...
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "42"})
	c.Assert(err, check.IsNil)
...
Do +2016-04-21T01:02:03Z +- +some summary
`
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--abs-time", "--last=install"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.DeepEquals, []string{})
	c.Check(s.Stdout(), check.Matches, expectedChange)
	c.Check(s.Stderr(), check.Equals, "")

	_, err = cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--abs-time", "--last=foobar"})
...
func (s *PebbleSuite) TestTasksLastQuestionmark(c *check.C) {
	...
	for i := 0; i < 2; i++ {
		rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--last=foobar?"})
		c.Assert(err, check.IsNil)
		c.Assert(rest, check.DeepEquals, []string{})
		c.Check(s.Stdout(), check.Matches, "")
		c.Check(s.Stderr(), check.Equals, "")

		_, err = cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--last=foobar"})
		if i == 0 {
			c.Assert(err, check.ErrorMatches, `no changes found`)
		} else {
			c.Assert(err, check.ErrorMatches, `no changes of type "foobar" found`)
...
func (s *PebbleSuite) TestTasksSyntaxError(c *check.C) {
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks", "--abs-time", "--last=install", "42"})
	c.Assert(err, check.NotNil)
	c.Assert(err, check.ErrorMatches, `cannot use change ID and type together`)

	_, err = cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"tasks"})
	c.Assert(err, check.NotNil)
