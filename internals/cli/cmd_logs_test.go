...
`[1:])
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs"})
	c.Assert(err, IsNil)
...
`[1:])
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "--format", "json"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, `
...
func (s *PebbleSuite) TestLogsInvalidFormat(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "--format", "invalid"})
...
`[1:])
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "-n2"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, `
2021-05-03T03:55:49.360Z [thing] log 1
...
`[1:])
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "-nall"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, `
2021-05-03T03:55:49.360Z [thing] log 1
...
func (s *PebbleSuite) TestLogsInvalidNumber(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "-ninvalid"})
...
`[1:])
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"logs", "-f"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, `
2021-05-03T03:55:49.360Z [thing] log 1
