...
func (s *PebbleSuite) TestVersion(c *C) {
	...
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"version"})
	c.Assert(err, IsNil)
...
func (s *PebbleSuite) TestVersionClientOnly(c *C) {
	...
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"version", "--client"})
	c.Assert(err, IsNil)
...
func (s *PebbleSuite) TestVersionExtraArgs(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"version", "extra", "args"})
