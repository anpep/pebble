...
func (s *PebbleSuite) TestRmExtraArgs(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"rm", "extra", "args"})
...
func (s *PebbleSuite) TestRm(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"rm", "/foo/bar.baz"})
	c.Assert(err, IsNil)
...
func (s *PebbleSuite) TestRmRecursive(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"rm", "-r", "/foo/bar"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
