...
func (s *PebbleSuite) TestMkdirExtraArgs(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "/foo", "extra", "args"})
...
func (s *PebbleSuite) TestMkdir(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "/foo"})
	c.Assert(err, IsNil)
...
func (s *PebbleSuite) TestMkdirFailsParsingPermissions(c *C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "-m", "foobar", "/foo"})
...
func (s *PebbleSuite) TestMkdirMakeParents(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "-p", "/foo/bar"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
...
func (s *PebbleSuite) TestMkdirPermissions(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "-m", "755", "/foo/bar"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
...
func (s *PebbleSuite) TestMkdirOwnerIDs(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "--uid", "1000", "--gid", "1000", "/foo/bar"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
...
func (s *PebbleSuite) TestMkdirOwnerNames(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "--user", "root", "--group", "wheel", "/foo/bar"})
	c.Assert(err, IsNil)
	c.Assert(rest, HasLen, 0)
	c.Check(s.Stdout(), Equals, "")
	c.Check(s.Stderr(), Equals, "")
...
func (s *PebbleSuite) TestMkdirFailsOnDirectory(c *C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"mkdir", "/foobar"})
