...
func (s *PebbleSuite) TestReplanExtraArgs(c *check.C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"replan", "extra", "args"})
...
func (s *PebbleSuite) TestReplan(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"replan"})
...
func (s *PebbleSuite) TestReplanNoWait(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"replan", "--no-wait"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
