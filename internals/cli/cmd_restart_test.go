...
func (s *PebbleSuite) TestRestart(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"restart", "srv1", "srv2"})
...
func (s *PebbleSuite) TestRestartFails(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"restart", "srv1", "srv3"})
...
func (s *PebbleSuite) TestRestartNoWait(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"restart", "srv1", "srv2", "--no-wait"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
