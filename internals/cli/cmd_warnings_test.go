...
func (s *warningSuite) TestNoWarningsEver(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"warnings", "--abs-time"})
	c.Assert(err, check.IsNil)
...
func (s *warningSuite) TestWarnings(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"warnings", "--abs-time", "--unicode=never"})
	c.Assert(err, check.IsNil)
	c.Check(rest, check.HasLen, 0)
	c.Check(s.Stderr(), check.Equals, "")
...
func (s *warningSuite) TestVerboseWarnings(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"warnings", "--abs-time", "--verbose", "--unicode=never"})
	c.Assert(err, check.IsNil)
	c.Check(rest, check.HasLen, 0)
	c.Check(s.Stderr(), check.Equals, "")
	c.Check(s.Stdout(), check.Equals, `
...
func (s *warningSuite) TestOkay(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"okay"})
	c.Assert(err, check.IsNil)
	c.Check(rest, check.HasLen, 0)
	c.Check(s.Stderr(), check.Equals, "")
	c.Check(s.Stdout(), check.Equals, "")
...
func (s *warningSuite) TestOkayBeforeWarnings(c *check.C) {
	_, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"okay"})
...
func (s *warningSuite) TestCommandWithWarnings(c *check.C) {
	...
	for warningCount, expectedWarning := range expectedWarnings {
		...
		rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"version"})
		c.Assert(err, check.IsNil)
...
func (s *warningSuite) TestExtraArgs(c *check.C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"warnings", "extra", "args"})
	c.Assert(err, check.Equals, cli.ErrExtraArgs)
	c.Check(rest, check.HasLen, 1)

	rest, err = cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"okay", "extra", "invalid arg"})
	c.Assert(err, check.Equals, cli.ErrExtraArgs)
	c.Check(rest, check.HasLen, 1)
}
