...
func (s *PebbleSuite) TestAutostartExtraArgs(c *check.C) {
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"autostart", "extra", "args"})
...
func (s *PebbleSuite) TestAutostart(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"autostart"})
...
func (s *PebbleSuite) TestAutostartNoWait(c *check.C) {
	...
	rest, err := cli.NewParser(cli.ParserMeta()).ParseArgs([]string{"autostart", "--no-wait"})
	c.Assert(err, check.IsNil)
	c.Assert(rest, check.HasLen, 0)
